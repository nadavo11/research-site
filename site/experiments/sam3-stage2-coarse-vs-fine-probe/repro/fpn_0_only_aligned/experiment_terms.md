...
- Run kind: `train`.
- Variant: `fpn_0_only`.
- Variant summary: Single-scale Stage-2 control using only the finest SAM pyramid level `fpn_0`.
...
- Selected levels for the active variant: `fpn_0`.
- Alignment reference level: `fpn_2` with aligned grid `(72, 72)`.
...
- Aligned level shape `fpn_0`: `(256, 72, 72)`.
- Single-scale comparison note: the selected level is resized to the globally coarsest discovered SAM grid before training and clustering so the scale comparison stays computationally tractable and uses one common spatial support.
