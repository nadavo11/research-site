...
- Run kind: `train`.
- Variant: `fpn_1_only`.
- Variant summary: Single-scale Stage-2 control using only the middle SAM pyramid level `fpn_1`.
...
- Selected levels for the active variant: `fpn_1`.
- Coarsest selected level: `fpn_1` with aligned grid `(144, 144)`.
...
- Aligned level shape `fpn_1`: `(256, 144, 144)`.
